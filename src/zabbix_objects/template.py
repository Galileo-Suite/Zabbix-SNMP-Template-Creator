...
class Template:
    def __init__(self, template_info_json: Dict[str, Any], snmp_item_json_list: List[Dict[str, Any]], snmp_trap_json_list: List[Dict[str, Any]], discovery_rule_tables: Dict[str, List[Dict[str, Any]]]):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future_items = executor.submit(SNMPItem.generate_snmp_items, snmp_item_json_list, self.name)
            future_traps = executor.submit(SNMPTrap.generate_snmp_traps, snmp_trap_json_list, self.name)
            future_discovery_rules = executor.submit(DiscoveryRule.generate_discovery_rules, discovery_rule_tables, self.name)
