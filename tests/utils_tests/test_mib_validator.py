...
def sample_excel_data():
    return {
        ...
        "MIB Data": pd.DataFrame({
            ...
            "Type": ["INTEGER", "STRING", "INTEGER", "NOTIFICATION-TYPE", "NOTIFICATION-TYPE"]
        })
    }
...
def mock_excel_file(sample_excel_data):
    with patch("pandas.ExcelFile") as mock_excel:
        ...
        mock_excel.return_value.parse.side_effect = lambda sheet_name: sample_excel_data[sheet_name]
        yield mock_excel

def test_extract_from_excel(mock_excel_file, sample_excel_data):
    with patch("pandas.read_excel") as mock_read_excel:
        mock_read_excel.side_effect = lambda file, sheet_name, **kwargs: sample_excel_data[sheet_name]
    ...
    assert template_info == {"Group": "Template/Networking", "Manufacturer": "CISCO", "Device":"Catalyst", "Model":"9300", "Macros":"", "Tags":""}
...
def test_preprocess_and_validate_success():
    input_data = [{"OID": ".1.2.3", "Name": "Item1"}, {"OID": ".1.2.4", "Name": "Item2"}]
    mib_data = [
...
def test_preprocess_and_validate_unmatched_data():
    input_data = [{"OID": ".1.2.3", "Name": "Item1"}, {"OID": ".1.2.5", "Name": "Item3"}]
...
def test_match_entries():
    ...
    matched, unmatched = MIBValidator._match_entries(input_data, mib_oid_dict, mib_name_dict)
...
@pytest.mark.parametrize("matched,unmatched,null,expected_output", [
    (
        [{"OID": ".1.2.3", "Name": "Item1"}],
        [],
        [],
